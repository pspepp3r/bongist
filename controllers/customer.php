...
class customer
{
    ...
    public static function register($fname, $lname, $email, $password)
    {
        ...
        if ($db->query($statement, $param)) {
            ...
            $msg = '<p style="color: #3E4095; font-size: 28px;"><span><b style="color: #288feb;">One more step, ' . $fname . '</b></span></p>
                <p>
    ...
    public static function forgot_password($email)
    {
        ...
        if (!self::check($email, 'email')) {
            ...
            if ($db->query($statement, $param)) {
                ...
                $username = self::check($email, 'customer_name')['customer_name'];
    ...
    public static function change_password($email, $old_password, $new_password, $repeat_password)
    {
        ...
        $account = self::check($email, 'password');
    ...
    public static function update($email, $fname, $lname, $phone, $photo = array())
    {
        ...
        if (
            $db->query("UPDATE accounts SET fname = :fname, lname = :lname, phone = :phone WHERE email = :email", array(
                'fname' => $fname,
                'lname' => $lname,
                'phone' => $phone,
                'email' => $email
    ...
    public static function remove($id)
    {
        ...
        // }
        respond::alert('success', '', (string) $id);
