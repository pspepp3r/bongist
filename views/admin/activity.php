	...
	<div class="page-head">
		<div class="container">
			...
			<div class="page-title">
				<h1><small>activities and report</small></h1>
			</div>
			<!-- END PAGE TITLE -->
				...
				<div class="col-md-6">
					...
					<div style="min-height:450px" class="portlet light ">
						<div class="portlet-title">
							<div class="caption caption-md">
								<i class="icon-bar-chart theme-font hide"></i>
								<span class="caption-subject theme-font bold uppercase">Activity Log</span>
							...
							<form action="" method="post" class="form-horizontal form-row-stripped">
											...
											<div class="form-group">
												...
												if(isset($_POST['blogs'])){
													...
													$q = $conn->prepare("INSERT INTO activitylog (log, subscription_id, timestamp) VALUES (:log, :sid, :tim)");

													$q->bindParam(':sid', $sid);
													$q->bindParam(':log', $desc);
													$q->bindParam(':tim', $date);
								 
													if ($q->execute()) {
													  ?>
												<div class="alert alert-success" >
												  <strong>Activity logged...</strong><br>
												  </div>
													<?php
												...
												<div class="alert alert-warning" >
												  <strong>Unable to log..</strong><br>
												  </div>
													<?php
													}
												}
											...
											<div class="form-group">
												<textarea <?php if($q4->rowCount() > 0){}else{echo "disabled";}?> required name="blog" placeholder="write a log" class="form-control" rows="6"><?php if(isset($_POST['blog'])){echo $_POST['blog'];}?></textarea>
											</div>
											...
											<div class="form-actions">
												<div class="row">
													<div class="">
														<button type="submit" <?php if($q4->rowCount() > 0){}else{echo "disabled";}?> name="blogs" class="btn btn-block btn-lg green"><i class="fa fa-check"></i> Submit</button>
													</div>
												</div>
											</div>
										</form>
										<hr/>
										<?php
											$q6 = $conn->prepare("SELECT * FROM activitylog WHERE Subscription_id = :id ORDER BY id DESC");
											$q6->bindValue(":id", $col['id']);
											$q6->execute();
											while($log = $q6->fetch()){
										?>
										<div class="alert alert-info al<?php echo $log['id'];?>">
										<button id="<?php echo $log['id'];?>" data-dismiss="alert" class="alertClose close">&times;</button>
											<p class="text-left">
											<?php
												echo $log['log'];
											?>
											...
											<br/>
											<p style="color:#fff; font-weight:bold" class="text-left">
											<?php
												echo date("F d, Y", $log['timestamp']);
											?>
											</p>
										</div>										
										<?php 
